...
* Commands in parenthesis are optional. Brackets are not but '|' means 'or'.

Flags: Flags are options which are designated with a dash e.g. -p. Not all 
	flags have to be used in the same command.
EOF
...
-t ("New title"): Listing title. Leaving no option will return the old title
...
-v ["NM description"]: Vinyl/CD condition. Must lead with a code like M or VG+,
	...
	NOTE! When using a minus (NM-), it must be escaped like: NM\\-
-j ["M description"]: Vinyl jacket/CD case condition. Same rules as -v.
-l auction: List as an auction instead of fixed price.
-upc ["12345678"]: UPC code of item.
...
Examples:
list -p 45.65 -v NM Looks great! -d Limited edition -i 602557093735
list "stone sour house of gold and bones 2"
EOF
		end
